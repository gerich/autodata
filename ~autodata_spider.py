...
class AutodataSpyderSpider(Spider):
    ...
    def __block(self, request):
        while (not request.meta['completed']):
            time.sleep(1)    

    def parse(self, response):
        marks = Selector(response).xpath('//ul[@id="all-manufacturers-list"]/li')
        marks = [marks[2]] + [marks[5]] + [marks[18]]
        
        self.last_result = []
        for mark in marks:
            mark_item = MarkItem()
            mark_item['link'] = mark.xpath('@manufacturer_id').extract()[0]
            a = mark.xpath('a')
            mark_item['name'] = a.xpath('text()').extract()[0].strip()
            yield mark_item

            url = self.start_urls[0] + 'ajax/' + mark_item['link']
            request = self.make_requests_from_url(url)
            request.callback = self.parse_models
            request.meta['mark'] = mark_item
                       
            yield request
            for model_item in self.last_result:
                request = self.__make_model_menu_request(mark_item, model_item)
                yield request

    def __make_model_menu_request(self, mark, model):
            request = FormRequest(
    ...
    def parse_models(self, response):
        ...
        for model in models:
            ...
            self.last_result.append(item)
            yield item
        response.request.meta['completed'] = True
